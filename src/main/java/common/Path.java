...
public class Path implements Iterable<String>, Comparable<Path>, Serializable {
	private static final long serialVersionUID = 8936401280143137641L;
	String path;
	String root = "/";
	...
	public Path(Path path, String component) {
		...
		this.path = Paths.get(path.getPath().toString(), component).normalize()
				.toString();
	}
	...
	public Iterator<String> iterator() {
		...
		return myPathIterator.iterator();
	}
	...
	public static Path[] list(File directory) throws FileNotFoundException {
		if (Files.exists(Paths.get(directory.getAbsolutePath()).normalize(),
		...
		Path[] ls = new Path[(int) directory.listFiles().length];
		int count = 0;
		for (File file : directory.listFiles()) {
			ls[count] = new Path(file.getPath());
		}
		return ls;
	}
	...
	public Path parent() {
		...
		return new Path(
				new File(this.getPath()).getParentFile().toPath().toString());
	}
	...
	public int compareTo(Path other) {
		return Paths.get(this.path).normalize()
				.compareTo(Paths.get(other.getPath()).normalize());
	}
	...
	public boolean equals(Object other) {
		return this.path == other.toString();
	}
	...
	public Path[] getParentPathList() {
		java.nio.file.Path nioPath = Paths.get(this.path);
		Path[] parentPathList = new Path[nioPath.getNameCount() - 1];
		int parentCount = nioPath.getNameCount() - 1;
		for (int i = 1; i < parentCount; i++) {
			parentPathList[i] = new Path(nioPath.subpath(0, i).toString());
		}
